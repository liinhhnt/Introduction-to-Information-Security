...
# 2: CFB
# 3: OFB
# Let d = student ID % 4 = 20200349 % 4 = 1
# --> Implement the CBC operation mode and apply it to DES.
...
# function implement DES encryption with CBC mode
# input: plaintext: 8-byte char string
#        subkeys: 16 subkeys (48-bit binary string)
#        iv: 64-bit binary string
# output: ciphertext: 64-bit binary string
def des_encrypt(plaintext, subkeys, iv):
    block = char_to_bits(plaintext)
    block = xor(block, iv)
    ...
    ciphertext = permute(right_half + left_half, FINAL_PERMUTATION)
    return ciphertext
...
def main():
    ...
    iv = input("Enter initial value of 8 length: ")
    print()
    ...
    # Checking if iv is valid or not
    if len(iv) != 8:
        print("Invalid Initial Value. IV should be of 8 length (8 bytes).")
        return
    ...
    # convert iv to bit string
    iv = char_to_bits(iv)

    for block in plaintext8byteBlocks:
        # apply cbc operation mode: use result of previous as IV of current block
        iv = des_encrypt(block, subkeys, iv)
        result += iv
